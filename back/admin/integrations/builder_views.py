...
class IntegrationBuilderMakeActiveUpdateView(AdminPermMixin, View):
    def post(self, request, *args, **kwargs):
        integration = get_object_or_404(Integration, id=self.kwargs["pk"])
        integration.is_active = True
        integration.save()
        messages.success(request, _("Integration has been activated"))
        return HttpResponseRedirect(reverse_lazy("settings:integrations"))
